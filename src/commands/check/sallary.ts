...
const plusDesc = ['PLUS CONFIDENCIALID.','PACTO NO COMPETENCIA', 'CESION PROP.INDUSTR.', 'CESION PRO.IND.'];
...
command
  ...
  .version(VERSION)
  .description(
    'Check the sallary calculated per month',
  )
  .action(
    actionRunner(async () => {
      const payroll = readPayroll();
      const headers: string[] = ['Month', 'Sallary', 'Antigüedad', 'Mejoras', 'Pluses', 'Sodexo', 'Paga Extra', 'Total Gross', 'Anual Gross'];
      const tableByMonth = new Table({
        head: headers,
      });
      payroll.forEach((entries, month) => {
        const sallary = entries.find((entry) => sallaryDesc.includes(entry.desc))?.income ?? 0;
        const antiguedad = entries.find((entry) => antiguedadDesc.includes(entry.desc))?.income ?? 0;
        const extra = entries.find((entry) => extraDesc.includes(entry.desc))?.income ?? 0;
        ...
        const total = plus.plus(convenio).plus(antiguedad).plus(sallary).plus(extra).plus(sodexo);
        tableByMonth.push([
          month,
          sallary.toString(),
          antiguedad.toString(),
  ...
  return entries.filter((entry) => descriptions.includes(entry.desc)).reduce(
    (sum, entry) => sum.plus(entry.income),
    new Decimal(0),
  )
}
